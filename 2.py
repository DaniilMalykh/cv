...
def calcul_off(image1, image2) -> [int, int] or None:
    left_top_image1, right_bottom_image1 = find_object(image1)
    left_top_image2, right_bottom_image2 = find_object(image2)
...
image1 = np.array(read_image('img1.txt'))
image2 = np.array(read_image('img2.txt'))

offset = calcul_off(image1, image2)
if offset:
    print(f'Смещение равно {offset[0]} от X и {offset[1]} от Y')
else:
    print('Одно изображение отличается')