...
def nominal_resolution(image, object_size) -> float or None:
...
for file_name in (os.listdir(DIRECTORY_PATH)):
    ...
    if os.path.isfile(full_path) and re.search(r'figure', file_name):
        ...
        nom_res = nominal_resolution(image, float(size))
        if nom_res:
            print(f'Файл {file_name}, номинальное разрешение {nom_res:.3f} пикселей в мм')
        else:
            print(f'Ничего не найдено {file_name}')